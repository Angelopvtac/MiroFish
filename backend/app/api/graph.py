...
def reset_project(project_id: str):
    """
    Reset project status (for rebuilding the knowledge graph)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    Endpoint 1: Upload files, analyze and generate ontology definitions
...
def build_graph():
    """
    Endpoint 2: Build a knowledge graph from a project_id
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently being built. Please do not submit again. Add force: true to force a rebuild.",

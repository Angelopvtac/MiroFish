...
# Adding this prefix prevents agents from calling tools and forces a plain text reply
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in the backend/scripts/ directory and are no longer
    ...
    try:
        ...
        # The following statuses indicate that preparation work has been completed:
        # - ready: preparation complete, can run
        # - preparing: if config_generated=True, preparation is done
        # - running: simulation is running, preparation was completed earlier
        # - completed: simulation finished, preparation was completed earlier
        # - stopped: simulation stopped, preparation was completed earlier
...
def prepare_simulation():
    """
    Prepare the simulation environment (async task; LLM intelligently generates all parameters)
    ...
    Features:
    - Detects already-completed preparation work to avoid duplicate generation
    ...
    4. Intelligently generate simulation configuration using LLM (with retry mechanism)
    ...
    try:
        ...
        # This allows the frontend to immediately know the expected total agent count after calling /prepare
        try:
            logger.info(f"Synchronously fetching entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick entity read (no edge information needed — just counting)
...
def get_prepare_status():
    ...
    1. Query an in-progress task's progress via task_id
    ...
    try:
        ...
        # If simulation_id is provided, check first whether preparation is already complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation has not started. Call /api/simulation/prepare to begin.",
                        "already_prepared": False
                    }
                })
            return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    If multiple reports match, returns the most recent one (sorted by created_at).
...
def get_simulation_history():
    ...
    Used for the homepage history section. Returns a simulation list enriched with
    project name, description, and other information.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read actual user-set round count from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-set total_rounds if available, otherwise use recommended round count
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g. file modification time, whether generation is in progress)
    - Returns partial information even if configuration is not fully generated yet
...
def get_simulation_config(simulation_id: str):
    """
    Get the simulation configuration (full LLM-generated configuration)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation configuration not found. Please call /prepare first."
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file not found. Please call /prepare first."
            }), 404
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": f"Unknown script: {script_name}. Allowed scripts: {allowed_scripts}"
            }), 400
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: maximum simulation rounds, to cap long simulations
            "enable_graph_memory_update": false,   // optional: whether to update agent activities to the Zep graph in real time
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete; check whether a simulation process is actually running
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is currently running. Call /stop first, or use force=true to force restart."
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get the current round's actions (recent_actions shows only the latest round)
        ...
        # recent_actions shows only the current latest round's content for both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for the frontend to display the progress bar and timeline view
...
def get_agent_stats(simulation_id: str):
    ...
    Used for the frontend to display agent activity rankings and action distribution
    """
    try:
...
def interview_agent():
    ...
    (i.e., after the simulation loop has finished and it is waiting for commands)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if not specified in dual-platform simulation: both platforms are interviewed
    ...
    try:
        ...
        # Optimize prompt — add prefix to prevent the agent from calling tools
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: in dual-platform simulations, each agent is interviewed on both platforms
    ...
    try:
        ...
        # Optimize each interview item's prompt — add prefix to prevent tool calls
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What is your overall opinion on this?",  // required, question (same for all agents)
            "platform": "reddit",                   // optional, specify platform (twitter/reddit)
                                                    // if not specified: in dual-platform simulations, each agent is interviewed on both
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, return only this agent's interview history
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation so it can gracefully exit
    the command-waiting mode.

    Note: This differs from /stop. The /stop endpoint forcibly terminates the process,

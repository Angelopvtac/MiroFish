...
class SimulationRunner:
    ...
    # Knowledge graph memory update configuration
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, used to limit overly long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update activities to the Zep knowledge graph
        graph_id: str = None  # Zep knowledge graph ID (required when graph update is enabled)
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            max_rounds: Maximum simulation rounds (optional, used to limit overly long simulations)
            enable_graph_memory_update: Whether to dynamically update agent activities to the Zep knowledge graph
            graph_id: Zep knowledge graph ID (required when graph update is enabled)
        ...
        if enable_graph_memory_update:
            ...
            try:
                ...
                logger.info(f"Knowledge graph memory update enabled: simulation_id={simulation_id}, graph_id={graph_id}")
            except Exception as e:
                logger.error(f"Failed to create knowledge graph memory updater: {e}")
        ...
        try:
            ...
            # This fixes issues in third-party libraries (like OASIS) that open files without specifying encoding
            env = os.environ.copy()
            ...
            # Set working directory to the simulation directory (databases and other files are generated there)
            # Use start_new_session=True to create a new process group, so os.killpg can terminate all child processes
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to ensure termination when server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Stop knowledge graph memory updater
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ZepGraphMemoryManager.stop_updater(simulation_id)
                    logger.info(f"Knowledge graph memory update stopped: simulation_id={simulation_id}")
                except Exception as e:
                    logger.error(f"Failed to stop knowledge graph memory updater: {e}")
    ...
    def _read_action_log(
        ...
        Returns:
            New read position
        """
        # Check if knowledge graph memory update is enabled
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round information (from round_end events)
                                    ...
                                    # Overall round count: take the maximum of the two platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall simulated time: take the maximum of the two platforms
                            ...
                            # If knowledge graph memory update is enabled, send activity to Zep
                            if graph_updater:
    ...
    def get_all_actions(
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old format files should already have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up (avoid logging useless messages for empty process lists)
        ...
        # Stop all knowledge graph memory updaters first (stop_all logs internally)
        try:
            ZepGraphMemoryManager.stop_all()
        except Exception as e:
            logger.error(f"Failed to stop knowledge graph memory updaters: {e}")
        cls._graph_memory_enabled.clear()

        # Copy dictionary to avoid modifying it during iteration
    ...
    def register_cleanup(cls):
        ...
        Called when the Flask application starts up to ensure all simulation processes
        ...
        # If not in debug mode, this environment variable is absent, but cleanup should still be registered
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: For dual-platform simulations, interview both platforms and return combined result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default to interviewing only on the Twitter platform
                - "reddit": Default to interviewing only on the Reddit platform
                - None: For dual-platform simulations, interview each agent on both platforms
            timeout: Timeout in seconds
    ...
    def close_simulation_env(
        ...
        try:
            ...
            # Timeout may indicate the environment is in the process of closing
            return {
                "success": True,

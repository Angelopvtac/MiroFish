...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert an activity to a text description suitable for sending to Zep

        Uses natural language description format so Zep can extract entities and
        relationships from it. No simulation-related prefix is added to avoid
        ...
        # Return in "agent_name: activity description" format with no simulation prefix
    ...
    def _describe_like_post(self) -> str:
        """Like a post — includes the post content and author"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post — includes the post content and author"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost — includes the original post content and author"""
    ...
    def _describe_quote_post(self) -> str:
        ...
        if quote_content:
            base += f" with the comment: \"{quote_content}\""
        return base
...
class ZepGraphMemoryUpdater:
    ...
    to the Zep graph in real time. Activities are grouped by platform; once
    BATCH_SIZE activities have accumulated per platform they are sent to Zep
    in a batch.

    All meaningful actions are updated to Zep. The action_args field contains
    full context information such as:
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full context information (e.g. post text, usernames, etc.).
            ...
            try:
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        # Check whether the platform has reached the batch size
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send a batch of activities to the Zep graph (merged into one text block)

...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-queries plus an integrated analysis
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (by sentence boundary, not hard cut)
...
class ZepToolsService:
    ...
    4. interview_agents - In-depth interview (interview simulated Agents, gather multi-perspective viewpoints)

    [Basic Tools]
    - search_graph - Graph semantic search
    - get_all_nodes - Get all nodes from the graph
    - get_all_edges - Get all edges from the graph (with temporal information)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges from the graph (with pagination, including temporal information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all graph edges, then filters to those connected to the specified node.
    ...
    def get_entity_summary(
        ...
        # Search for information related to the entity
    ...
    def get_simulation_context(
        ...
        # Filter entities that have a real type (not pure Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function — automatically decomposes the question
        ...
        # Fetch details for all relevant entities (no limit, complete output)
        entity_insights = []
        node_map = {}  # For building relationship chains later
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional question analysis expert. Your task is to decompose a complex question into multiple sub-questions that can each be independently observed in a simulated world.
...
Return the list of sub-questions in JSON format."""
    ...
    def panorama_search(
        ...
        Gets a full overview including all related content and historical/expired information:
        1. Fetch all related nodes
        2. Fetch all edges (including expired/invalidated ones)
        3. Categorize current active information and historical information

        This tool is best suited for scenarios requiring a full picture of events
        or tracking how something has evolved over time.
        ...
        for edge in all_edges:
            ...
            # Determine if the edge is expired/invalidated
            is_historical = edge.is_expired or edge.is_invalid

            if is_historical:
                # Historical/expired facts — add temporal markers
    ...
    def interview_agents(
        ...
        [Important] This feature requires the simulation environment to be running (OASIS environment must not be closed)
        ...
        - Need real responses from simulated Agents (not LLM-simulated)
        ...
        try:
            # Build the batch interview request list (no platform specified — dual-platform interview)
            interviews_request = []
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean up any tool-call JSON wrapping
                ...
                clean_text = re.sub(r'(?:Question|问题)\s*\d+[：:]\s*', '', clean_text)
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Build the path to the persona file directory
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                ...
                - selected_indices: Index list of the selected Agents (for API calls)
                - reasoning: Reasoning behind the selection
        """
...
3. Select a diverse range of viewpoints (e.g. supporters, opponents, neutral parties, subject matter experts)
...
Please select up to {max_agents} Agents most suitable for interview and explain your selection rationale."""
...
2. Different roles may give different answers to the same question
3. Cover multiple dimensions: facts, opinions, feelings, etc.
4. Natural language, like a real interview
5. Keep each question under 50 words — concise and direct

...
- Does not close the environment immediately after simulation completes — enters command-wait mode
...
# This addresses the problem of third-party libraries (e.g. OASIS) reading
# files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls that don't specify an encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')

    # Reconfigure standard output streams to UTF-8 (fixes Chinese garbling in console)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force the default encoding (affects the default encoding for the open() function)
    # Note: ideally this is set at Python startup; setting at runtime may not take effect.
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        This fixes issues where third-party libraries (e.g. OASIS) open files
...
# Load .env file from the project root (contains LLM_API_KEY and other config)
...
# Add the filter immediately at module load time, before any camel code runs
...
# Available Twitter actions (INTERVIEW excluded — it can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (INTERVIEW excluded — it can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms simultaneously, returns combined result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items requiring both platforms
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Get an agent_id -> entity_name mapping from simulation_config

    This allows actions.jsonl to display real entity names rather than placeholders like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the database with enriched context

    Args:
        db_path: Path to the database file
        last_rowid: The highest rowid processed last time (using rowid instead of created_at,
    ...
    Returns:
        ...
        - new_last_rowid: New highest rowid
    """
    actions = []
...
def _enrich_action_context(
    ...
    try:
        ...
        # Enrichment failure should not affect the main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for parallel simulation speed-up:
    ...
    in parallel simulation, increasing concurrency capacity.
    ...
    # Select which LLM to use based on parameters and availability
    ...
    # Fall back to config if .env does not specify a model
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Get real Agent name mapping from config (use entity_name, not the default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed rowid (using rowid avoids created_at format differences)
    ...
    for round_num in range(total_rounds):
        # Check if we received an exit signal
        ...
        # Always log round start, even if there are no active agents
        if action_logger:
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional, to cap overly long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation completes — do not enter command-wait mode'
    )
...
def setup_signal_handlers(loop=None):
    ...
    it waits for interview commands. On receiving a termination signal, we need to:
    ...
    def signal_handler(signum, frame):
        ...
        # Only force exit if we receive a repeated signal
        else:
            print("Force exiting...")
            sys.exit(1)

...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Curvature range grows with more edges
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center so isolated node clusters gather to center
  ...
  // Compute curved path for a link
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that departs and returns to the same node
      const loopRadius = 30
    ...
    // Compute Bezier control point - dynamically adjusted by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset proportional to distance, ensuring curves are visibly distinct
    // More edges → larger proportion of distance used for offset
  ...
  // Compute the midpoint of a curve (used for label positioning)
  ...
  // Link labels background (white background for legibility)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether actual dragging has started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)

        if (!d._isDragging && distance > 3) {
          // Only restart simulation upon first confirmed drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation cool down if an actual drag occurred
        if (d._isDragging) {

      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show overflow hint if there are more files -->
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} more files
            </div>
          </div>
          <!-- Empty placeholder when there are no files -->
...
const selectedProject = ref(null)  // Currently selected project (used by modal)
let observer = null
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute card count for current row to center each row
    ...
    // Expand downward with increased spacing from the title
...
// Get progress CSS class based on round progress
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record the latest intent, even during animation)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to be executed (may have been superseded)
          ...
          // Release lock after animation and handle any pending state changes
          setTimeout(() => {
            isAnimating = false

            // After animation, check for new pending state changes
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before executing to avoid too-rapid transitions
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// When using keep-alive, reload data on component activation
onActivated(() => {
  loadHistory()
})
...
/* Square grid with fixed spacing using CSS background pattern */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor at top-left; height changes only extend the bottom, preserving existing grid position */

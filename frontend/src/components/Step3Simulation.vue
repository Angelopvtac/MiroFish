...
// Info Plaza simulated elapsed time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Topic Community simulated elapsed time
...
// Reset all state (used to restart simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // stop any previously running polls
}
...
// Track previous round per platform to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation has completed (via runner_status or platform completion flags)

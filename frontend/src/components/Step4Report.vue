...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button in the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract stats - match "相关预测事实: X条" format
...
const parseInterview = (text) => {
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract answers - split by Twitter and Reddit platform
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (for legacy format with only one platform marker)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Independent platform selection state per question-answer pair
    ...
    // Try to split answers by question numbering
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fall back to "number." format if no match
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only shown when there are real answers on both platforms)
...
// Currently active step (shown in header)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading second-level heading (## xxx) since section title is rendered outside
  ...
  // Clean <p><br> immediately before block elements (caused by extra blank lines)
  ...
  // Fix numbering for non-contiguous ordered lists: when single-item <ol>s are separated by paragraph content, keep counter incrementing
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic handled after loop in nextTick
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If done, scroll to top; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
